...
import com.nvim.R;
import com.nvim.adapter.ContactBaseAdapter;
...
import com.nvim.utils.CharacterParser;
...
public class ImContactFragment extends ImMainBaseFragment implements OnTouchingLetterChangedListener,
	...
	private List<Object> getDepartmentTabSortedList(Map<String, ContactEntity> contacts) {
	...
	private void locateDepartmentImpl(String departmentId) {
		...
		//
		// final int position =
		// departmentAdapter.locateDepartment(department.title);
		...
		EntityList entityList = new EntityList(contactList) {
			...
			public void onSearchImpl(String key) {
				...
				for (Object obj : list) {
					...
					if (contact.pinyinElement.pinyin.contains(key) || contact.name.contains(key)) {
						searchList.add(obj);
					}
				}
	...
	private void contactTabOnContactsReady() {
		...
		EntityList entityList = new EntityList(contactList) {
			...
			public int getPinYinFirstCharacter(int position) {
				...
				if (contact.pinyinElement.pinyin == null || 0 == contact.pinyinElement.pinyin.length()) {
					return 0;
				}
	...
	private void onGroupReady() {
		...
		// todo efficiency
		// List<Object> groupList = new
		// ArrayList<Object>(imService.getGroupManager().getNormalGroupList());
