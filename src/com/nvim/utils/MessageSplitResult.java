...
public class MessageSplitResult {
	...
	public MessageSplitResult(MessageInfo info, byte[] content) {
		...
		if (CacheHub.getInstance().getChatUser() != null) {
			logger.d("target user id = " + CacheHub.getInstance().getChatUser().getUserId() + " name = "
	...
	private void split() {
		...
		while (!(strOriginContent.length() == 0)) {
			...
			if (nStart < 0) {// 没有头
				...
				if (nEnd < 0) {// 没有尾
					...
					String matchString = subContentString.substring(0, nEnd + end.length());
					addMessage(matchString);

					strOriginContent = subContentString.substring(nEnd + end.length());
				}
			}
	...
	public void addMessage(String strContent) {
		...
		if (strContent.startsWith(SysConstant.MESSAGE_IMAGE_LINK_START)
			...
			String imageUrl = strContent.substring(SysConstant.MESSAGE_IMAGE_LINK_START.length());
			imageUrl = imageUrl.substring(0, imageUrl.indexOf(SysConstant.MESSAGE_IMAGE_LINK_END));

			logger.d("recv an image message: image url = " + imageUrl);

			msgInfo.setUrl(imageUrl.length() == 0 ? null : imageUrl);
			msgInfo.setDisplayType(SysConstant.DISPLAY_TYPE_IMAGE);
	...
	private void setAudioDetail() {
		...
		if (msgLen < 4) {// 错误判断
			...
			String audioSavePath = FileUtil.saveAudioResourceToFile(audioContent);
