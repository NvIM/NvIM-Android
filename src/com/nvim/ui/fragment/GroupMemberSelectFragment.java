...
import com.nvim.adapter.GroupManagerAdapter.OnDeleteItemListener;
	...
	private void initRes() {
		...
		topRightTitleTxt.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				//
				// int sessionType =
				// GroupMemberSelectFragment.this.sessionInfo.getSessionType();
				...
				// logger.d("tempgroup#generateTempGroupName:%s",
				// tempGroupName);
				//
				// ShowDialogForTempGroupname(groupMgr, memberList,
				// tempGroupName);
				//
				// } else if (sessionType == IMSession.SESSION_TEMP_GROUP) {
				// List<String> addingMemberList =
				// gridViewHelper.getAdapter().getAddingMemberList();
				// List<String> removingMemberList =
				// gridViewHelper.getAdapter().getRemovingMemberList();
				//
				// imService.getGroupManager().changeTempGroupMembers(sessionInfo.getSessionId(),
			...
			private void ShowDialogForTempGroupname(final IMGroupManager groupMgr, final List<String> memberList,
					String defaultName) {
		...
		gridViewHelper.onInit(curView, R.id.group_manager_grid, getActivity(), false, null,
...
//			}
//
//			private void handleContactItemClick(View view, Context ctx, int position) {
...
//		contactAdapter.add(0, entityList);
	...
	private void handleChangeTempGroupMemberResult(Intent intent) {
		...
		if (!ok) {
			Toast.makeText(getActivity(), getString(R.string.change_temp_group_member_failed), Toast.LENGTH_SHORT)
					.show();
			return;
		}
		...
		// todo eric adding an notification to the chat window, tell that
		// "members are changed, right now, members are 'a', 'b', 'c' "
