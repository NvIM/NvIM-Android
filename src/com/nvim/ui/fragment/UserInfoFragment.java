...
public class UserInfoFragment extends MainFragment implements OnIMServiceListner {
	...
	private void initBaseProfile(IMService imService, SessionInfo sessionInfo) {
		...
		final ContactEntity contact = null;
		// imService.getContactManager().findContact(sessionInfo.getSessionId());
		if (contact == null) {
		...
		portraitImageView.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				// intent.putExtra(SysConstant.KEY_AVATAR_URL,
				// contact.avatarUrl);
				// intent.putExtra(SysConstant.KEY_IS_IMAGE_CONTACT_AVATAR,
				// true);
				//
	...
	private void initDetailProfile(IMService imService, SessionInfo sessionInfo) {
		...
		final ContactEntity contact = null;// imService.getContactManager().findContact(sessionInfo.getSessionId());
		if (contact == null) {
			logger.d("detail#no such contact id:%s", sessionInfo.getSessionId());
			return;
		}

		DepartmentEntity department = null;// imService.getContactManager().findDepartment(contact.departmentId);
		if (department == null) {
			return;
		}
