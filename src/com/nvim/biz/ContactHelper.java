...
public class ContactHelper {
	...
	// * @Description 请求最近联系人
	// */
	// public static void reqRecentContact(final Handler msgHandler, final
	// Handler uiHandler) {
	...
	public static void loadRecentInfoList(Handler msgHandler, Context context) {
		try {
			...
			int friendNo = CacheHub.getInstance().getFriendIdList(CacheHub.getInstance().getLoginUserId(), context)
					.size();
			...
			while (itr.hasNext()) {
				...
				MessageInfo msgInfo = CacheHub.getInstance().getLastMessage(friendId);
				if (null != msgInfo) {
					recentInfo.setLastContent(msgInfo.getMsgOverview());
					recentInfo.setLastTime(Long.valueOf(msgInfo.getUpdated() > 0 ? msgInfo.getUpdated() : msgInfo
							.getCreated()) * 1000);
				...
				int unreadCount = CacheHub.getInstance().getUnreadCount(friendId);
	...
	public static int resetContactMessagetState(RecentInfo recentinfo) {
		...
		int readCount = CacheHub.getInstance().clearUnreadCount(recentinfo.getUserId());
		// 设置当前选中用户本地DB中消息为已读
		CacheHub.getInstance().updateMsgReadStatus(CacheHub.getInstance().getLoginUserId(), recentinfo.getUserId(),
