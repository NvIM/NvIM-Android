...
import com.nvim.biz.MessageDistCenter;
import com.nvim.config.SysConstant;
import com.nvim.db.biz.DataBaseHelper;
...
public class CacheModel {
	 ...
	 * @param userId 用户ID
	 */
	private Boolean initLastMessageAndCount(List<String> friendIds, String ownerId) {
		// try {
	...
	private Boolean reviseMsgLoadingStatusInDB(String onwerId) {
		return dbHelper.updateMsgStatusFromStatus(onwerId, SysConstant.MESSAGE_STATE_FINISH_FAILED,
	...
	public MessageInfo getLastMessage(String friendUserId) {
		...
		// MessageCacheImpl.getInstance().get(
		// friendUserId);
		// if (null == msgInfo) {
	 ...
	 * @return MessageInfo
	 */
	public List<MessageInfo> pullMsg(String userId, String friendUserId, int msgId, int offset, int size) {
	...
	public int checkAndDeleteIfNeed() {
		...
		if (0 < timeLine) {
			File audioDirFile = new File(CommonUtil.getSavePath(SysConstant.FILE_SAVE_TYPE_AUDIO));
			File imageDirFile = new File(CommonUtil.getSavePath(SysConstant.FILE_SAVE_TYPE_IMAGE));
